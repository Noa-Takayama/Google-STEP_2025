# あまりにも遅かったので, 1秒ごとに 2-opt 法の進捗状況を表示してもらうようにした
        ...
        for i in range(num_cities - 1):
            for j in range(i + 2, num_cities):
                ...
                if current_time - last_update_time > 1.0:
                    # \rで行頭に戻り、同じ行に上書き表示する
                    # flush=Trueで即座にターミナルに表示させる
                    progress_msg = f"    -> 2-opt search in progress... (Checking node i={i}/{num_cities})"
                    sys.stdout.write(f"\r{progress_msg:<80}") # 固定幅でクリア
                    sys.stdout.flush()
...
def iterated_local_search(cities, max_iterations=100, time_limit=60):
    ...
    # 1. 初期化
    print("[Phase 1] Creating initial random route...")
