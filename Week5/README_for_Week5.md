...
-   **局所最適解への陥り**: ランダムな初期経路から開始する `solver_ils.py` では、質の低い局所最適解に囚われ、制限時間内にスコアが全く改善されないケースがありました。
-   **フィードバック不足**: 計算に最も時間を要する2-opt法の実行中に、プログラムが正常に動作しているかどうかのフィードバックがなく、進捗を把握するのが困難でした。

### 改善

これらの課題を解決するため、2段階の改善を行いました。

#### 計算状況の可視化 (`solver_ils2.py`)

「フィードバック不足」の問題に対し、時間のかかる`local_search_2opt`関数の実行中に、1秒ごとに進捗を表示する機能を追加しました。
